document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateCard() {
        // Ensure background image is loaded before drawing
        backgroundImage.onload = function() {
            // Set canvas size to match the card's visual size
            const cardWidth = card.offsetWidth;
            const cardHeight = card.offsetHeight;

            canvas.width = cardWidth * 2; // Double the resolution for high quality
            canvas.height = cardHeight * 2;
            ctx.scale(2, 2); // Scale canvas for high resolution
